...
namespace ReModCE.Core
{
    public class ReaderPatches
    {
        ...
        private static unsafe void DoPatch<T>(ProcessModule module, int offset, T patchDelegate, out T delegateField)
        ...
        private static unsafe void FloatTransferPatch(IntPtr reader, float* result, byte* fieldName)
        {
            ...
            if (CanReadBadFloats || *result > MaxAllowedValueBottom && *result < MaxAllowedValueTop) return;
        ...
        {
            if (Configuration.GetAvatarProtectionsConfig().AntiAvatarAudioMixerCrash && !CanReadAudioMixers)
            {
                return IntPtr.Zero;
            }
        ...
        [ThreadStatic] private static int ourReaderOobDepth;
        ...
        {
            [FieldOffset(0x70)] public NodeContainer** Subs;
